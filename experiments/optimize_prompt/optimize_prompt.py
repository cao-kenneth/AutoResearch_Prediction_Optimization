...
BASELINE_PREDICTIONS_PATH = EXPERIMENT_DIR / "outputs" / "train_valid_predictions.csv"
START_RUN_DIR = EXPERIMENT_DIR / "outputs" / "run_001"
MODEL_SCRIPT = EXPERIMENT_DIR / "model.py"
...
def load_starting_point():
    run_predictions_path = START_RUN_DIR / "predictions.csv"
    run_prompt_path = START_RUN_DIR / "prompt.txt"
    run_metrics_path = START_RUN_DIR / "metrics.json"

    if run_predictions_path.exists() and run_prompt_path.exists():
        best_prompt = run_prompt_path.read_text(encoding="utf-8")
    ...
    best_prompt = PROMPT_FILE.read_text(encoding="utf-8")
...
def run_model():
    ...
    predictions_path = Path(run_path) / "predictions.csv" if run_path else None
    ...
    for iteration in range(start_iteration, stop_iteration):
        ...
        try:
            ...
            if new_score is not None and new_score < best_score:
                ...
                PROMPT_FILE.write_text(previous_prompt, encoding="utf-8")
